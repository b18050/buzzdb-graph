...
public:
    GraphIndex() {
        // Initialize all entries as non-existing by default
        std::cout << "GraphIndex initialized!" << std::endl;
    }
    void insertUser(int user_id) {
        if (hashTable.find(user_id) == hashTable.end()) {
            hashTable[user_id] = std::vector<int>();
        }
    }

    void insertOrUpdate(int user_id1, int user_id2) {
        hashTable[user_id1].push_back(user_id2);
        hashTable[user_id2].push_back(user_id1);
    }

    const std::vector<int>& getFriendsOfAUser(int user_id) const {
        static const std::vector<int> emptyList;
        auto it = hashTable.find(user_id);
        if (it != hashTable.end()) {
            return it->second;
        }
        return emptyList;
    ...
    void print() const {
        for (const auto& entry : hashTable) {
            std::cout << "User ID: " << entry.first << ", Friends: ";
            for (int friend_id : entry.second) {
                std::cout << friend_id << " ";
            }
            std::cout << std::endl;
        }
...
std::vector<int> findMutualFriends(int user_id1, int user_id2, const GraphIndex& graph_index) {
    auto friends1 = graph_index.getFriendsOfAUser(user_id1);
    auto friends2 = graph_index.getFriendsOfAUser(user_id2);
...
std::vector<int> findMutualFriendsTwoLevels(int user_id1, int user_id2, const GraphIndex& graph_index) {
    // Helper function to perform BFS up to 2 levels
    auto getFriendsUpToTwoLevels = [&graph_index](int user_id) -> std::unordered_set<int> {
        std::unordered_set<int> visited; // To track visited nodes
        std::queue<std::pair<int, int>> bfsQueue; // Pair of (current node, level)
        bfsQueue.push({user_id, 0});
        ...
        while (!bfsQueue.empty()) {
            ...
            if (level >= 2) {
                continue; // Stop exploring beyond the second level
            }

            for (int friend_id : graph_index.getFriendsOfAUser(current)) {
                if (visited.find(friend_id) == visited.end()) {
                    visited.insert(friend_id);
                    bfsQueue.push({friend_id, level + 1});
                }
            }
        }

        visited.erase(user_id); // Remove the starting user from the set
        return visited;
    };

    // Get friends up to 2 levels for both users
    auto friends1 = getFriendsUpToTwoLevels(user_id1);
    auto friends2 = getFriendsUpToTwoLevels(user_id2);

    // Find mutual friends
    std::vector<int> mutual_friends;
    for (int friend_id : friends1) {
        if (friends2.find(friend_id) != friends2.end() && friend_id != user_id1 && friend_id != user_id2) {
            mutual_friends.push_back(friend_id);
        }
    }

    // Optional: Sort mutual friends if needed
    std::sort(mutual_friends.begin(), mutual_friends.end());
    return mutual_friends;
}


std::vector<int> getFriends(int user_id, const HashIndex& graph_index) {
    return graph_index.getFriendsOfAUser(user_id);
}
        ...
        for (size_t page_itr = 0; page_itr < num_pages; page_itr++) {
            ...
            for (size_t slot_itr = 0; slot_itr < MAX_SLOTS; slot_itr++) {
                if (slot_array[slot_itr].empty == false){
                    ...
                    // Build indexes
                    tuple_count++;
                    graph_index.insertOrUpdate(key, value);
                }
            }
        }
    ...
    void executeComplexQueries() {
        ...
        for (int i = 1; i <= 99; i++) {
            for(int j=1;j<=99;j++) {
                int user_id1 = i;
                int user_id2 = j;
                auto mutual_friends = findMutualFriends(user_id1, user_id2, graph_index);
        ...
        for (int i = 1; i <= 99; i++) {
            for(int j=1;j<=99;j++) {
                int user_id1 = i;
                int user_id2 = j;
                auto mutual_friends_second = findMutualFriendsTwoLevels(user_id1, user_id2, graph_index);
